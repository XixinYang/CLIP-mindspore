...
def difference(ms_file,torch_file):
    ...
    if torch_variable.shape!=ms_variable.shape:
        raise ValueError(f'{ms_variable} has shape {ms_variable.shape} while {torch_variable} has different shape of {torch_variable.shape}.')
    ...
    print(f'{os.path.basename(ms_file).replace(".txt",": ")}\n abs_mean: {abs_mean}\n abs_max: {abs_max}\n rel_mean: {rel_mean}\n rel_max: {rel_max}\n\n')
